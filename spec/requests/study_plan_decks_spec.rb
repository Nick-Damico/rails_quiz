...
RSpec.describe "StudyPlanDecks", type: :request do
    ...
    # FIX: Create condition where a deck cannot be added to the study plan
    xcontext "when the deck cannot be added to the study plan" do
      before do
        create(:study_plan_deck, study_plan: study_plan, deck: deck)
      end
      ...
      it "sets an error flash message" do
        ...
        expect(flash[:alert]).to eq(
          "There was an issue adding this deck to your study plan. Refresh page and try again."
        )
      end
    end
  end

xdescribe "DELETE /study_plans/:study_plan_id/study_plan_decks/:id" do
  let!(:study_plan_deck) do
    create(:study_plan_deck, study_plan: study_plan, deck: deck)
  end

  subject(:destroy_study_plan_deck) do
    delete study_plan_deck_path(study_plan_deck)
  end

  it "removes the deck from the study plan" do
    expect { destroy_study_plan_deck }
      .to change(StudyPlanDeck, :count).by(-1)
  end

  it "renders the study plan show page" do
    destroy_study_plan_deck
  ...
  it "sets a success flash message" do
    ...
    expect(flash[:notice]).to eq("Deck removed from study plan.")
  end
end
